...
export function extractShareTitle(html) {
  const tags = String(html).match(/<meta\b[^>]*>/gi) || [];
  for (const tag of tags) {
    const name = [...tag.matchAll(/\bname\s*=\s*["']([^"']+)["']/gi)][0]?.[1];
    if (String(name).toLowerCase() !== "share-title") continue;
    const content = [...tag.matchAll(/\bcontent\s*=\s*["']([^"']+)["']/gi)][0]?.[1];
    if (content) return unescapeHtml(content).trim();
  }
  return "";
...
export function grokOgHeadTags({
  ...
  const overrideTitle = String(titleOverride ?? "").trim();
  const title = overrideTitle || resolveOgTitle(site, appName, host, documentTitle);
  const publicHost = resolvePublicHost(host);
  const tags = [
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  next = insertAfterHeadOpen(
    ...
    grokOgHeadTags({
      ...
      titleOverride: extractShareTitle(html),
    }).join(""),
  );
