...
function asIncrements(value: unknown): Array<5 | 10 | 50> {
  const allowed = new Set([5, 10, 50]);
  let raw: unknown[] = [];
  ...
  const out = raw
    ...
    .filter((n): n is 5 | 10 | 50 => allowed.has(n as 5 | 10 | 50));
  return out.length > 0 ? out : [5, 10, 50];
}
...
export const getListing = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const detail: ListingDetail = {
      ...
      durationHours: asInt(row.duration_hours),
      bidIncrements: asIncrements(row.bid_increments),
...
export const createListing = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const bidIncrements = asIncrements(data.bidIncrements);
    const imagesJson = JSON.stringify(images);
    const incrementsJson = JSON.stringify(bidIncrements);
    const rows = await sql<{ id: number }>`
      insert into listings (
        seller_id, title, description, images, website_url, website_name, youtube_url,
        starting_price, current_price, duration_hours, bid_increments, starts_at, ends_at, status
      ) values (
        ${context.userId},
        ${title},
        ${description},
...
export const placeBid = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const allowed = asIncrements(listing.bid_increments);
    if (!allowed.includes(increment as 5 | 10 | 50)) {
      throw new Error(`叫價只可加 ${allowed.map((n) => `$${n}`).join("、")}`);
    }
